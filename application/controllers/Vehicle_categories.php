...
class Vehicle_categories extends MY_Controller
{	
    ...
    **/ 
	function index()
	{
		$this->data['ajaxrequest'] = array(
			'url' => site_url().'vehicle_categories/ajax_get_list',
			'disablesorting' => '0,3',
		);
		...
		$this->data['content'] 		= "admin/vehicle_categories/vehicle_categories";
    ...
    **/ 
    function ajax_get_list()
    {
		if($this->input->is_ajax_request())
		{
			...
			$columns = array('category','status');	
			
			$query 	= "SELECT * from ".TBL_PREFIX.TBL_VEHICLE_CATEGORIES." WHERE id!=''";
			...
			if(!empty($records)) {
				...
				foreach($records as $record)
				{
					...
					$dta ='';
					$dta .= '<span>';
					$dta .= form_open(site_url().'vehicle_categories/addedit');
					...
					$str .= '<a data-toggle="modal" class="btn btn-danger" data-target="#deletemodal" onclick="delete_record('.$record->id . ',\''.site_url().'vehicle_categories/delete_record'.'\')">
				  ...
				  <div class="slideThree slideBlue">
					...
					<input type="checkbox" value="' . $record->id . '" id="status_' . $record->id . '" name="check_' . $record->id . '" onclick="statustoggle(this, \'' .  site_url().'vehicle_categories/statustoggle' .'\')"'.$checked . '/>
    ...
    **/ 
    function addedit()
    {
		if(isset($_POST['addedit_vc']))
		{
			
			$this->check_isdemo(site_url().'vehicle_categories');
			...
			if($this->form_validation->run() == TRUE) 
			{
				...
				if($this->input->post('id') > 0)
				{
					...
					if($this->base_model->update_operation($data, TBL_VEHICLE_CATEGORIES, $where))
					{
				...
				{
					$id = $this->base_model->insert_operation_id($data, TBL_VEHICLE_CATEGORIES);
					if($id)
					{
						$msg .= $this->lang->line('details_saved_successfully');
						$status = 0;
				...
				if($msg != '')
					...
					redirect(site_url().'vehicle_categories', REFRESH);
			}
		}
		
		
		
		$pagetitle = $this->lang->line('add_vehicle_category');
		if(isset($_POST['edit_record']))
		{
			$id = $this->input->post('id');
			if($id > 0)
			{
				$pagetitle = $this->lang->line('edit_vehicle_category');
				$record = $this->base_model->fetch_records_from(TBL_VEHICLE_CATEGORIES, array('id' => $id));
				if(empty($record)) 
				{
					$this->prepare_flashmessage($this->lang->line('no_details_found'), 2);
					redirect(site_url().'vehicle_categories');				
				}
		...
		$this->data['active_class']  = "vehicle";   
		$this->data['content'] 		 = 'admin/vehicle_categories/addedit';
		$this->_render_page(TEMPLATE_ADMIN, $this->data);
    }
    ...
    **/ 
    ...
    {	
		if (!DEMO) {
            ...
            {			
                $ids = explode(',', $id);
                $details = $this->base_model->fetch_records_from_in(TBL_VEHICLE_CATEGORIES, 'id', $ids);
                
                if(!empty($details))
                {
                    
                    if ($this->base_model->delete_record_in(TBL_VEHICLE_CATEGORIES, 'id', $ids)) {
    ...
    /**
     * Change status of Vehicle Category
     *
     *
     * @return boolean
    **/ 
    function statustoggle()
    {
		if (!DEMO) {
            ...
            if($this->input->post())
            {
                ...
                $this->base_model->update_operation_in( $filedata, TBL_VEHICLE_CATEGORIES, 'id', explode(',', $id) );
